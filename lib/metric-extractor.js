...
const K = {
  ...
  cacheDirty:       'serverStatus.wiredTiger.cache.tracked dirty bytes in the cache in the checkpoint',
...
function diskDelta(cur, prev, suffix) {
  ...
  for (const key of Object.keys(cur)) {
    if (key.startsWith('systemMetrics.disks.') && key.endsWith(suffix)) {
...
function extractSignals(frames) {
  for (let i = 0; i < frames.length; i++) {
    ...
    const p = i > 0 ? frames[i - 1].metrics : m; // prev = self for frame 0

    const cacheMax = get(m, K.cacheMax) || 1;
    const readTotal = get(m, K.wtReadTotal) || 128;
    const writeTotal = get(m, K.wtWriteTotal) || 128;
    const connTotal = get(m, K.connCurrent) + get(m, K.connAvailable);

    // CPU
    const cpuUserD   = Math.max(0, get(m, K.cpuUser)   - get(p, K.cpuUser));
    const cpuSysD    = Math.max(0, get(m, K.cpuSys)    - get(p, K.cpuSys));
    const cpuIdleD   = Math.max(0, get(m, K.cpuIdle)   - get(p, K.cpuIdle));
    const cpuIowaitD = Math.max(0, get(m, K.cpuIowait) - get(p, K.cpuIowait));
    const cpuTotal   = cpuUserD + cpuSysD + cpuIdleD + cpuIowaitD || 1;

    // Disk
    const diskReadBytes  = diskDelta(m, p, '.read_bytes');
    const diskWriteBytes = diskDelta(m, p, '.write_bytes');

    // Checkpoint: spike when a checkpoint finishes (cumulative ms jumps)
    const checkpointDelta = Math.max(0, get(m, K.checkpointMs) - get(p, K.checkpointMs));

    const insertRate  = i > 0 ? delta(m, p, K.insert)  : 0;
    const queryRate   = i > 0 ? delta(m, p, K.query)   : 0;
    const updateRate  = i > 0 ? delta(m, p, K.update)  : 0;
    const deleteRate  = i > 0 ? delta(m, p, K.delete)  : 0;
    const commandRate = i > 0 ? delta(m, p, K.command) : 0;
    const getmoreRate = i > 0 ? delta(m, p, K.getmore) : 0;

    frames[i].signals = {
      cache_fill_pct:        get(m, K.cacheBytes) / cacheMax,
      dirty_pct:             get(m, K.cacheDirty) / cacheMax,
      app_eviction_rate:     i > 0 ? delta(m, p, K.appEvict) + delta(m, p, K.modEvict) : 0,
      total_eviction_rate:   i > 0 ? delta(m, p, K.appEvict) + delta(m, p, K.modEvict) : 0,
      read_ticket_util:      get(m, K.wtReadOut)  / readTotal,
      write_ticket_util:     get(m, K.wtWriteOut) / writeTotal,
      read_tickets_out:      get(m, K.wtReadOut),
      write_tickets_out:     get(m, K.wtWriteOut),
      read_tickets_total:    readTotal,
      write_tickets_total:   writeTotal,
      conn_util:             connTotal > 0 ? get(m, K.connCurrent) / connTotal : 0,
      conn_current:          get(m, K.connCurrent),
      conn_available:        get(m, K.connAvailable),
      ...
      net_in_mbps:           i > 0 ? delta(m, p, K.netBytesIn)  / 1e6 : 0,
      net_out_mbps:          i > 0 ? delta(m, p, K.netBytesOut) / 1e6 : 0,
      queue_readers:         get(m, K.queueReaders),
      queue_writers:         get(m, K.queueWriters),
      queue_depth:           get(m, K.queueReaders) + get(m, K.queueWriters),
      mem_resident_mb:       get(m, K.memResident),
      mem_virtual_mb:        get(m, K.memVirtual),
      checkpoint_duration_ms: checkpointDelta,
      exec_target_throughput: get(m, K.execTarget),
      exec_admitted:          get(m, K.execAdmitted),
    };
...
module.exports = { extractSignals, computeSubsystemHealth };
