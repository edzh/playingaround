...
// Pull host identity out of a type-0 metadata chunk. The metadata doc is a
// plain BSON subdocument (not compressed) containing getCmdLineOpts/hostInfo/
// buildInfo as captured at mongod/mongos startup.
function extractHostInfo(metaDoc) {
  const num = v => (v == null ? null : Number(v.valueOf ? v.valueOf() : v));
  const md       = metaDoc || {};
  const parsed   = md.getCmdLineOpts?.parsed || {};
  const hostname = md.hostInfo?.system?.hostname || 'unknown';
  const port     = num(parsed.net?.port) || 27017;
  const rsName   = parsed.replication?.replSetName || null;
  const sharding = parsed.sharding || {};

  const role = sharding.configDB                  ? 'mongos'
             : sharding.clusterRole === 'configsvr' ? 'configsvr'
             : sharding.clusterRole === 'shardsvr'  ? 'shardsvr'
             : rsName                               ? 'replset'
             : 'standalone';
  ...
  return {
    ...
    rsName,
    role,
    uniqueLabel: `${rsName ? rsName + '/' : ''}${hostname}:${port}`,
    mongoVersion: md.buildInfo?.version || null,
  };
}
...
async function parseFile(buffer, opts = {}) {
  ...
  for (const { doc: chunkDoc } of iterateChunks(buffer)) {
    ...
    if (type === 0) {
      if (!hostInfo) {
        try { hostInfo = extractHostInfo(chunkDoc.doc); } catch (_) { /* keep null */ }
      }
      continue;
    }
    if (type !== 1) continue;
  ...
  return { frames: allFrames, metricKeys: metricKeys || [], hostInfo };
}

module.exports = { parseFile, flattenDocInOrder, readUvarint, extractHostInfo };
